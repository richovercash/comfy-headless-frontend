...
const ComfyService = {
  ...
  createFluxWorkflow({
    prompt,
    negativePrompt = '', //low quality, bad anatomy, blurry, pixelated, distorted, deformd'
    steps = 28,
    inputImageUrl = null,
    reduxImageUrl = null,
...
// This function will merge the working LoRA integration with the advanced workflow
// Update comfyService.js to include this improved version

  /**
   * Create an advanced Flux workflow with LoRA support, Redux styling, and depth conditioning
  ...
  createFluxAdvancedWorkflow({ 
    ...
    loras = [] // Added LoRA support
  }) {
    console.log("Creating advanced Flux workflow with params:", { 
      prompt, steps, reduxStrength, useDepth, filenamePrefix, loraCount: loras.length
    });
    
    // Log LoRA details if present
    if (loras && loras.length > 0) {
      console.log("Using LoRAs in advanced workflow:", loras.map(lora => ({
        name: lora.name || lora.file_path,
        file_path: lora.file_path,
        model_strength: lora.model_strength,
        clip_strength: lora.clip_strength
    ...
    let workflow = {
      ...
      "89": {
        "inputs": {
          "text": fullPrompt, // Use combined prompt with activation words
          "clip": [
            "80",
            0
          ]
      ...
      "105": {
        "inputs": {
          "text": activationWords,//fullPrompt.split(',').slice(0, 5).join(', '), // Use first few tags with activation words
          "clip": [
            "80",
            0
          ]
    ...
    // CRITICALLY IMPORTANT: Add LoRAs to the workflow if provided
    if (loras && loras.length > 0) {
      try {
        console.log(`Adding ${loras.length} LoRAs to advanced workflow`);
        
        // Try EasyLoraStack method first
        const updatedWorkflow = this._applyEasyLoraStackToWorkflow(workflow, loras);
        
        // Check if LoRAs were successfully added
        const hasLoraStack = Object.values(updatedWorkflow).some(node => 
          node.class_type === "easy loraStack"
        );
        
        if (hasLoraStack) {
          console.log("Successfully added LoRAs using EasyLoraStack method");
          workflow = updatedWorkflow;
        } else {
          // If EasyLoraStack failed, try the FluxLoraLoader fallback
          console.log("EasyLoraStack method failed, trying FluxLoraLoader fallback");
          const fallbackWorkflow = this._applyFluxLoraLoaderFallback(workflow, loras);
          
          // Check if fallback method added LoRAs
          const hasFluxLoras = Object.values(fallbackWorkflow).some(node => 
            node.class_type === "FluxLoraLoader"
          );
          
          if (hasFluxLoras) {
            console.log("Successfully added LoRAs using FluxLoraLoader fallback");
            workflow = fallbackWorkflow;
          } else {
            console.error("Both LoRA integration methods failed!");
          }
        }
      } catch (error) {
        console.error("Error applying LoRAs to advanced workflow:", error);
        // Continue with the original workflow if there was an error
      }
    }
